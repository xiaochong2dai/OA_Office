...
namespace SDLX.BLL
{
	...
	public class ERPUser
	{
        ...
        public void UserLogin(string MyUserName, string MyUserPwd)
        {
            ...
            {
                if (MyUserPwd == SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "UserPwd"))
                {
                    if (SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "IfLogin").Trim()=="是")
                    {
                        System.Web.HttpContext.Current.Session["UserID"] = SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "ID");
                        System.Web.HttpContext.Current.Session["UserName"] = SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "UserName");
                        System.Web.HttpContext.Current.Session["JiaoSe"] = SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "JiaoSe");
                        System.Web.HttpContext.Current.Session["Department"] = SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "Department");
                        System.Web.HttpContext.Current.Session["TrueName"] = SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "TrueName");
                        System.Web.HttpContext.Current.Session["ZhiWei"] = SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "ZhiWei");
                        System.Web.HttpContext.Current.Session["QuanXian"] = SDLX.DBUtility.DbHelperSQL.GetStringList("select QuanXian from ERPJiaoSe where JiaoSeName in(" +"'"+SDLX.Common.DataValidate.ValidateDataRow_S(MyDataRow, "JiaoSe").Replace(",","','")+"'"+ ")");
                        //写登陆日志
